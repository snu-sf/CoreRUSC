...
Next Obligation. eapply Events.match_traces_nil_l in NIL. ss. Qed.
Next Obligation. eapply Events.match_traces_nil_r in NIL. ss. Qed.
...
Lemma bsim_to_xsim
    ...
    <<XSIM: xsim L1 L2 order top1 top1 i0 st_src0 st_tgt0>>.
Proof.
...
Proof.
  ...
  - i. destruct props.(xsim_initial_states_sim); eauto.
...
Proof.
  ...
  { rr in H. rr. des. esplits; eauto. inv H. econs; eauto.
    i. exploit (sd_determ_at0 t1 s0 E0 s2); eauto. i; des. eapply match_traces_nil_r in H; subst. exploit H2; eauto. i; clarify.
    exploit (sd_determ_at0 t2 s4 E0 s2); eauto. i; des. eapply match_traces_nil_r in H; subst. exploit H3; eauto. }
Qed.
...
Proof.
  ...
  { econs; eauto. inv H. econs; eauto. i. determ_tac sd_determ_at0. eapply match_traces_nil_l in H; subst. clear STEP1.
    determ_tac sd_determ_at0. eapply match_traces_nil_l in H; subst. esplits; eauto. rewrite <- H3; ss. rewrite <- H4; ss. }
  { eapply DStar_E0_SDStar; eauto. }
  ss.
Qed.

Lemma xsim_to_generalized_xsim
      L1 L2 index (order: index -> index -> Prop) ss_src ss_tgt i0 st_src0 st_tgt0
      (PUBEQ: forall id, Senv.public_symbol L1.(symbolenv) id = Senv.public_symbol L2.(symbolenv) id)
      (XSIM: xsim L1 L2 order ss_src ss_tgt i0 st_src0 st_tgt0):
    <<XSIM: GENMT.xsim L1 L2 order ss_src ss_tgt i0 st_src0 st_tgt0>>.
Proof.
  revert_until ss_tgt. pcofix CIH. i. pfold. ii. punfold XSIM. repeat spc XSIM. des.
